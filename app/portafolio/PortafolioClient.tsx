...
import { ArrowUpRight, Filter, Search, X, ChevronDown, Check } from "lucide-react";
...
/** Mapea focus -> valor exacto del filtro Service */
function focusToService(focus?: string): string | null {
  const f = (focus ?? "").toLowerCase();
  if (f === "basico") return "Plan Básico";
  if (f === "medio") return "Plan Medio";
  if (f === "premium") return "Plan Premium";
  if (f === "landing" || f === "landing48h" || f === "48h") return "Landing page 48 horas";
  return null;
}

type Props = { focus?: string };

export default function PortafolioClient({ focus }: Props) {
  ...
  // ✅ aplica focus desde /portafolio?focus=medio (solo una vez)
  useEffect(() => {
    const mapped = focusToService(focus);
    if (mapped) setServiceFilter(mapped);
  }, [focus]);
    ...
    <div className="section">
      ...
      <section className="max-w-4xl">
        ...
        <div className="mt-8 grid gap-3 lg:grid-cols-[1.4fr_0.8fr_0.8fr_auto] items-stretch">
          ...
          <div ref={serviceWrapRef} className="relative rounded-2xl border border-white/10 bg-black/40 px-4 py-3">
            ...
            <button
              ...
              className={cx(
                ...
                serviceOpen ? "border-sky-400/40 bg-slate-950/50" : "border-white/10 bg-transparent",
            ...
            >
              ...
              <ChevronDown className={cx("w-4 h-4 text-slate-300 transition", serviceOpen && "rotate-180")} />
            </button>
                    ...
                    return (
                      <button
                        ...
                        className={cx(
                          ...
                          // ✅ HOVER celeste claro (no blanco)
                          "hover:bg-sky-400/20 hover:border-sky-300/25"
                        )}
                      >
                        <span className="truncate">{s}</span>
                        {active ? <Check className="w-4 h-4 text-sky-200" /> : <span className="w-4 h-4" />}
                      </button>
                    );
                  })}
                </div>
          ...
          {/* status filter (puedo pasarlo a dropdown igual si quieres) */}
          ...
          <button
            ...
            className={cx(
              ...
              hasFilters ? "border-white/15 bg-white/5 text-slate-200 hover:bg-white/10" : "border-white/10 bg-black/30 text-slate-500 cursor-default"
            )}
            disabled={!hasFilters}
      ...
      <section className="mt-16 text-center">
        ...
        <p className="mt-3 text-[11px] text-slate-500">
          Si tu caso es institucional (roles, continuidad, contenidos), recomendamos partir por Plan Medio o Premium.
        </p>
      </section>
    </div>
  );
